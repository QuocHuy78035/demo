...
import { Button, Form, Input, message, Modal, Result, Spin, Table, Tag } from 'antd';
...
const postTodo = async (title: string): Promise<void> => {
    await axios.post("https://dummyjson.com/todos/add", { title, completed: false, userId: 13 });
}
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (completed: boolean) => {
                return <div style={{ maxWidth: 20 }}>
                    {completed ? <Tag icon={<CheckCircleOutlined />} color="success">Completed</Tag> : <Tag icon={<SyncOutlined spin />} color="processing">
                        processing
                    </Tag>
                    }
                </div>
